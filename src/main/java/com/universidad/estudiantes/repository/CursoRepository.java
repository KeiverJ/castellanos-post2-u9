...
/**
 * Repositorio JPA para Curso.
 * Usa JOIN FETCH para cargar estudiantes en la misma consulta (evita N+1).
 */
@Repository
public interface CursoRepository extends JpaRepository<Curso, Long> {

    // JOIN FETCH para cargar estudiantes evitando el problema N+1
    @Query("SELECT c FROM Curso c LEFT JOIN FETCH c.estudiantes")
    List<Curso> findAllConEstudiantes();

    // Cargar un curso especifico con sus estudiantes
